...
data all_data;
    ...
    * fist row for a firm and the year is 1993, the avg_at is 0;
    if first.gvkey and fyear=1993 then avg_at = 0;
    * first row for a firm and the year is not 1993, then the avg_at is current at;
    else if first.gvkey and fyear ~= 1993 then avg_at = at;
    else 
...
data scaled_values;
    ...
    * pre-tax book income;
    PTBI = SUM(pi, -1*MII)/avg_at;
    * year ahead pre-tax book income;
    PTBI_AHEAD = SUM(ptbi_lead, -1*mii_lead)/avg_at;
    * pre-tax cash flow;
    ...
    *current tax;
    curtax = SUM(TXT, -1*TXDI)/0.35;
    /* curtax = (TXT - TXDI)/0.35; */
run;
